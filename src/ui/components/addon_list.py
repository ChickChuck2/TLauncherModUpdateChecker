...
class AddonList(ctk.CTkFrame):
    """Painel esquerdo com toolbar + lista scrollável de AddonCards."""
    ...
    def _refresh(self):
        ...
        if not filtered:
            ctk.CTkLabel(self.scroll, text="Nenhum addon encontrado.",
                         text_color="#6e7681", font=ctk.CTkFont(size=12)).pack(pady=30)
            self.lbl_count.configure(text="0")
            return

        for idx, addon in enumerate(filtered):
